...
const CommitteeTotal = () => {
    ...
    const fetchData = () => {
      fetch('https://djsnss-grainathon25.onrender.com/grain-a-thon2025/comm')
        ...
        .then(data => {
          ...
          setCommittees(getSortedArr(arr));
        })
        .catch(error => {
  ...
  const totalScore = Number(
    committees.reduce((acc, curr) => acc + (Number(curr.quantity) || 0), 0).toFixed(1)
  );
          ...
          <table className="w-full text-white text-xl font-bold text-center bg-white bg-opacity-10 rounded-xl font-silkscreen overflow-hidden">
            <thead>
              <tr>
                ...
                <th className="py-1 px-2">Committee</th>
            ...
            <tbody>
              <AnimatePresence>
                {committees.map((item, idx) => (
                  ...
                  >
                    <td className="py-1 px-2">{item.rank}</td>
                    <td className="py-1 px-2">{item.comm}</td>
                    <td className="py-1 px-2">{item.dept}</td>
